  ...
  const handleAddressFetch = async () => {
    ...
    try {
      ...
      if (data.status === "OK" && data.results[0]) {
        ...
        if (!street) {
          ...
          if (reverseGeocodeResponse.ok) {
            ...
            if (reverseData.status === "OK" && reverseData.results[0]) {
              ...
              street = reverseAddressComponents.find((comp) =>
                comp.types.includes("route")
        ...
        const formattedAddress = `${street || "Onbekende straat"} ${houseNumber}, ${city}, ${province}, ${country}`;
      ...
      setErrorMessage("Er is een fout opgetreden bij het ophalen van het adres.");
    } finally {
      setIsLoading(false);
    }
  };
