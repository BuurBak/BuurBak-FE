...
const AanbodList = () => {
  ...
  const TrailerArray = SearchOrFilter({ searchTerm: inputValueSearch, filterType: inputValueType, filterDate: inputValueWhen, filterWhere: inputValueWhere });
  ...
  useEffect(() => {
    if(inputValueWhere === undefined || "" || null){
      ...
      fetch("https://api.pdok.nl/bzk/locatieserver/search/v3_1/suggest?q=" + inputValueWhere + "&rows=100&fq=*:*")
      .then((res) => res.json())
      .then((data) => {
        setCallData(data.response.docs);
        setNames(callData?.filter(item => item.type === 'woonplaats').map(item => item.weergavenaam.split(',')[0].trim()));
          ...
          <div className="flex flex-row flex-wrap gap-3">
            ...
            <ThemeProvider theme={customTheme(outerTheme)}>
              ...
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker label="Wanneer"
                  value={inputValueWhen}
                  onChange={(newValue) => { setInputValueWhen(newValue) }}
                  slotProps={{
                    field: {
                      clearable: true, onClear: () => setdateCleared(true),
                    },
                  }} />
