...
export default function TrailerImagesUpload({ onFilesChange }: FileUploadProps) {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      setFiles((prev) => [...prev, ...imageFiles].slice(0, amountOfImages));
    },
    [amountOfImages]
  );
  ...
  return (
    <div className="w-full">
      {files.length < amountOfImages ? (
        <div
          className={`border-2 border-dashed rounded-lg p-6 cursor-pointer transition-colors ${isDragging ? "border-primary bg-primary/10" : "border-gray-300"
            } ${files.length >= amountOfImages ? "border-green-500" : ""}`}
